...
`
    await expect(loadComponent(code, "test", "Bar")).rejects.toThrow(
      /Export "Bar" not found/
    )
  })
  ...
  it("includes available exports in error message", async () => {
    ...
    await expect(loadComponent(code, "test", "Baz")).rejects.toThrow(
      /Available exports: Foo, Bar, default/
    )
  })
