...
import {
  ...
  getSourceContent,
  parseComponentUrl,
} from "../loader/sourceMapRegistry.ts";
    ...
    return (
      ...
      >
        <div className="stack-frame-header">
          ...
          {frame.isServerComponent && (
            <span className="stack-frame-badge">component</span>
          )}
        </div>
        <div className="stack-frame-location">
          <span className="stack-frame-file">{shortFile}</span>
          {displayLine > 0 && (
            <span className="stack-frame-line">:{displayLine}</span>
          )}
          {displayColumn > 0 && (
            <span className="stack-frame-column">:{displayColumn}</span>
          )}
        </div>
        {frame.sourceLine && (
          <pre className="stack-frame-source">{frame.sourceLine}</pre>
        )}
      </div>
    );
  };
