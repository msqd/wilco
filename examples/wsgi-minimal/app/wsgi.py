"""Pure WSGI application demonstrating wilco integration without a framework.

This module implements a raw WSGI application with:
...
# Route handlers
@router.get("/")
def product_list(environ: dict, params: dict[str, str]) -> tuple[str, str, bytes]:
    """Product list page."""
    ...
    return "200 OK", "text/html", html.encode("utf-8")


@router.get("/products/(?P<product_id>\\d+)")
def product_detail(environ: dict, params: dict[str, str]) -> tuple[str, str, bytes]:
    """Product detail page."""
    ...
    if product is None:
        ...
        return "404 Not Found", "text/html", html.encode("utf-8")
...
def serve_static(path: str) -> tuple[str, str, bytes, dict[str, str]]:
    """Serve a static file.
    ...
    if path.startswith("/static/"):
        ...
        return "404 Not Found", "text/plain", b"Not Found", extra_headers

    # Security: prevent directory traversal
    try:
        file_path = file_path.resolve()
        if not file_path.is_relative_to(base_dir.resolve()):
            return "403 Forbidden", "text/plain", b"Forbidden", extra_headers
    ...
    return "200 OK", content_type, content, extra_headers


# API endpoints for component bundles
def api_list_bundles() -> tuple[str, str, bytes, dict[str, str]]:
    """List all available bundles."""
    bundles = bundle_handlers.list_bundles()
    return "200 OK", "application/json", json.dumps(bundles).encode("utf-8"), {}


def api_get_bundle(name: str) -> tuple[str, str, bytes, dict[str, str]]:
    """Get bundled JavaScript for a component."""
    try:
        result = bundle_handlers.get_bundle(name)
    except ValueError:
        error = {"detail": f"Invalid component name: '{name}'"}
        return "422 Unprocessable Entity", "application/json", json.dumps(error).encode("utf-8"), {}
...
def api_get_metadata(name: str) -> tuple[str, str, bytes, dict[str, str]]:
    ...
    return "200 OK", "application/json", json.dumps(metadata).encode("utf-8"), {}
...
def app(environ: dict, start_response: Callable) -> Iterable[bytes]:
    ...
    if path.startswith("/static/") or path.startswith("/media/") or path.startswith("/wilco-static/"):
        status, content_type, body, extra_headers = serve_static(path)
    # Handle API endpoints
    elif path == "/api/bundles" and method == "GET":
        status, content_type, body, extra_headers = api_list_bundles()
    elif match := BUNDLE_JS_PATTERN.match(path):
        if method == "GET":
            name = match.group(1)
            status, content_type, body, extra_headers = api_get_bundle(name)
        else:
        ...
        if method == "GET":
            name = match.group(1)
            status, content_type, body, extra_headers = api_get_metadata(name)
        else:
            status, content_type, body = "405 Method Not Allowed", "text/plain", b"Method Not Allowed"
    else:
        # Route to handlers
        status, content_type, body = router.dispatch(method, path, environ)
